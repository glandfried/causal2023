...
Contra factual
\begin{equation*}
P(s^{\prime}, r | \text{do}(c = 1), s = 2 , \text{do}(c^{\prime}= 2))
\end{equation*}
\large
\only<7>{
\begin{tabular}{c|c|c|c|}
...
\begin{equation*}
P(s^{\prime}, r | \text{do}_{c^{\prime}}= 2, \text{do}(c = 1), s = 2) \ \
\end{equation*}
\large
\begin{tabular}{c|c|c|c|}
         &  $r1$  &  $r2$ & $r3$   \\ \hline
...
\begin{equation*}
P(y\,|\,\text{do}(t)) = \onslide<3>{\sum_w P(y\,|\, t, w) P(w)}
\end{equation*}
...
\only<4->{
\begin{textblock}{150}(5,14) \centering \small
\begin{flalign*}
& P(\text{Derrumbes}|do(\text{Intensidad del sismo})) \only<4>{\overset{?}{=}}\only<5>{\approx}\only<6>{=} \onslide<6>{\sum_{\text{Ciudad}}} \only<5-6>{P(\text{Derrumbes}|\text{Intensidad del sismo}, \text{Ciudad})} \onslide<6>{P(\text{Ciudad})}
&&
\end{flalign*}
\end{textblock}
}
...
\end{textblock}
 ...
 \begin{frame}[plain]
\begin{textblock}{160}(0,4)
 \centering \LARGE Backdoor Criterion\\
 \end{textblock}
 \vspace{0.75cm}

 \begin{flalign*}
\hspace{1.5cm} & P(y|\text{do}(t)) = \sum_w P(y|\text{do}(t), w) P(w|\text{do}(t)) \\[0.3cm]
& hhs &&
 \end{flalign*}
...
\only<2-3>{
\begin{textblock}{160}(0,74) \centering
\begin{equation*}
P( \text{Llamada} | do(\text{Entradera}) ) = \onslide<2>{\sum_{\text{Ciudad}} P(\text{Llamada}|\text{Entradera}, \text{Ciudad}) P(\text{Ciudad}) }
\end{equation*}
\end{textblock}
}
